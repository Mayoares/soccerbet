...
echo "<title>Kick-Tipp - Gruppe $group - mit Eingabefeld</title>";
echo "</head>";
echo "<body>";
...
if(isset($_POST["saveRanks"])){
	saveRanks($userName, $userId, $group);
	echo "<br>";
}
...
while($array=mysql_fetch_array($sqlResult))
{
	...
	$log=new logger();
	$log->info($sqluser);
	$tippedTeam = getTeamOnRank($userName, $group, $rankCnt);
	echo "<tr>";
	echo "<td>$rankCnt.</td>";
...
//		$saveMatchErrorText = saveMatch($i, $userName);
//		$matchnrPost=$i;
//	}
//}
...
while($array=mysql_fetch_array($sqlResult)){
	...
	$picname1=getPicName($teamName1);
	$picname2=getPicName($teamName2);
	...
	echo "<td> $date </td> <td> $formattedTime </td> <td>Spiel $matchnr &nbsp;&nbsp;</td> " .
			"<td><p><img src=\"../../general/pics/flags/europe/$picname1\" alt=\"Flagge-$teamName1\"></p></td>" .
			"<td><b>$teamName1</b> </td> <td>-</td> 
			<td><p><img src=\"../../general/pics/flags/europe/$picname2\" alt=\"Flagge-$teamName2\"></p></td>
			<td> <b>$teamName2</b></td>  
...
function getTeamOnRank($username, $group, $rank){
...
function saveMatch($matchnr, $userName){
	include_once("../../general/log/log.php5");
	$log=new logger();
...
function saveRanks($userName, $userId, $group){
	//$log=new logger();	
	...
	if(areAllTeamNamesDifferent($teamName1,$teamName2,$teamName3,$teamName4))
	{
		for($rankCnt=1;$rankCnt<=4;$rankCnt++)
		{
			...
			$shortTeamName=getShortName($teamName);
