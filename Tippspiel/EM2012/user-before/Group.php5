...
echo "<title>EM-Tipp - Gruppe $group - mit Eingabefeld</title>";
echo "</head>";
echo "<body>";
...
function insertUpdateMatchTipp($userName, $matchnr, $teamName1, $teamName2, $GoalsTeam1, $GoalsTeam2, $winner){
	$log=new logger();	
	$goaldiff=$GoalsTeam1-$GoalsTeam2;
	$table_groupmatchtipps=dbschema::groupmatchtipps;
	// columns: user 	matchnr 	goalsX 	goalsY 	winner 	goaldiff 	score
	$sqlinsert="INSERT INTO $table_groupmatchtipps VALUES ('$userName', '$matchnr', '$GoalsTeam1', '$GoalsTeam2', '$winner', '$goaldiff', NULL)";
	$log->info($sqlinsert);
	...
	if (!$sqlInsertResult) {
		...
		$sqlupdateMatch="UPDATE $table_groupmatchtipps SET " .
			...
			"winner = '$winner', " .
			"goaldiff=$goaldiff " .
			"WHERE `$table_groupmatchtipps`.`user` =  '$userName' AND `$table_groupmatchtipps`.`matchnr` =$matchnr";
...
function saveRanks($userName, $userId, $group, $dbutil){
	...
	if(areAllTeamNamesDifferent($teamName1,$teamName2,$teamName3,$teamName4))
	{
		...
		echo "<font color=\"green\">Platzierungen gespeichert.</font>";
		echo "<br>";
	}
	else
	{	
